...
setores |>
  ...
  stat_smooth(data = setores[setores$PGsemDoc|>is.na()|>not(),],
              aes(y = PGsemDoc, x = IRDA),
              colour = "blue",
              se = FALSE, 
              method = 'loess',
              formula = 'y ~ x')+
  stat_smooth(aes(y = PGcomDoc, x = IRDA),
              colour = "red", 
              se = FALSE, 
              method = 'loess', 
              formula = 'y ~ x')+
  stat_smooth(aes(y = IRPG, x = IRDA), 
              colour = "green",
              se = FALSE, 
              method = 'loess', 
              formula = 'y ~ x')+
...
## O efeito de ter documento é mais significativo nos setores de mais alta renda.

<TABLE WIDTH = 100%>
  <TR>
    <TD COLSPAN = 2>
Os Gráficos 8 e 9 mostram o IRPG conforme situação do cadastro do documento. No Gráfico 8 é possível perceber que ter o CPF cadastrado implica em um nível de pagamento mais elevado nos setores de mais alta renda. Isso se dá provavelmente pela falta de capacidade de pagamento da população de mais baixa renda (vide avaliaão sobre regressividade do imposto). Já o Gráfico 9 mostra mais uma vez: por um lado, que um conjunto significativo de setores de alta renda observam pagamentos da dívida mesmo sem documento registrado; por outro lado, a ausência de documento permite que os índices de pagamento caiam ao mesmo patamar dos setores de baixa renda.
    </TD>
  </TR><TR>
    <TD WIDTH = 50%>
```{r Gráfico 8}
